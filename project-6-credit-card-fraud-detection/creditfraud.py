...
# can comment it later on to train entire dataset instead of a few of them
df = df.sample(frac=0.1, random_state = 48)

# Print shape and description of the data
'''
st.checkbox() is used to show/hide a specific section in an app. It takes a single argument, which is the widget label. In this app, the checkbox is used to toggle a conditional statement. st.write() is the equivalent of print() command. It can write charts, dataframes, and simple text. For a cleaner look, st.sidebar is used. 
'''
...
# st.slider() widget can slide to change the output of the web app.
...
'''
Here feature importance method to select top features to train our model.
For complex and time taking functions that need to run only once we mark the function with Streamlit’s cache decorator @st.cache, whenever the function is called, streamlit checks the input parameters that you called the function with. If this is the first time streamlit has seen these parameters, it runs the function and stores the result in a local cache. When the function is called the next time, if those parameters have not changed, streamlit knows it can skip executing the function altogether. It just uses the results from the cache.
'''
...
# Calculating the Accuracy, Precision, Recall, F1-Score and the Matthews correlation coefficient for model evaluation.
