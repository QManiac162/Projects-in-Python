...
# Fetching historical market data for Apple (AAPL) and Dow Jones (DJI) for yesterday (1-minute intervals)
...
# print("\nBasic Financial data: ", yf.Ticker.financials)
# print("\nStock Actions like dividend and split: ", yf.Ticker.actions)
...
# Varibles to track daily context
...
def process_stock_update():
    ...
    if not data.empty and not dow_data:
        ...
        # safely remove the 1st row without causing index issues
        data = data.iloc[1:]
        ...
        # Limit the rolling window to 5 min for moving avg
        ...
        # calculate insights (moving avgs, Bollinger Bands, RSI, etc)
...
def get_market_open_duration(window):
    # extract current time from the last element of the window which returns a datetime.time object
    current_time = window.index[-1].time()

    # get the prev trading day's date
    previous_trading_day = datetime.today() - timedelta(days=1)

    # combine the previous trading day with the current time
    current_datatime = datetime.combine(previous_trading_day, current_time)

    # define the market opening time as 09:30:00 on the prev trading day
    ...
    get_market_open_duration = (current_datatime - market_start_time).total_seconds()/60

    return get_market_open_duration




# Function to calculate insights like moving avgs and trends
def calculate_insights(window, dow_window):
    if len(window) >= 5:
        # calculate 5-min rolling avg of the 'Close' prices
        rolling_avg = window['Close'].rolling(window=5).menu().iloc[-1]

        # calculate price change and volume change
        price_change = window['Close'].iloc[-1] - window['Close'].iloc[-2] if len(dow_window) >= 2 else 0
        volume_change = window['Volume'].iloc[-1] - dow_window['Volume'].iloc[-2] if len(dow_window) >= 2 else 0
        ...
        # calculate Exponential Moving Avg (EMA) and Bollinger Bands (with a 5 period window)
        ...
        # calculate Relative Strength Index (RSI) of there is are enough periods (14 is typical)
